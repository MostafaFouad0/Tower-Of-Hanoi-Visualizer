...
namespace Hanoi
{
    ...
    {   Dictionary<int ,PictureBox> mp=new Dictionary<int ,PictureBox>();
        ...
        void towerOfHanoi(int n, char from, char to, char tmp,ref int A,ref int B,ref int C)
        {
            if (n == 1)
            {
            ...
            towerOfHanoi(n - 1, from, tmp, to,ref A,ref B,ref C);
            MOVE(n, from, to,ref A, ref B, ref C);
            Thread.Sleep(speed);
            towerOfHanoi(n - 1, tmp, to, from,ref A,ref B,ref C);
        }
        ...
        {
            towerOfHanoi(x, 'A', 'C', 'B',ref A_count, ref B_count, ref C_count);
            int ret = 1;
            for (int i = 1; i <=x; i++)
            {
                ret = ret * 2;
            }
            MessageBox.Show("Number of Moves = " + ret);
        }
